...
IF (equipment_id NOT IN(SELECT ItemNumber FROM active_bookings)) THEN 
    SET booked_quantity =0;
ELSE
    SET booked_quantity = (SELECT SUM(NumberOfItems) FROM active_bookings GROUP BY equipment_id  HAVING ItemNumber = equipment_id);
END IF;
